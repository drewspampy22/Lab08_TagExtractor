...
public class TagExtractorFrame extends JFrame
{
    ...
    private void createDisplayPanel()
    {
        ...
        chooserButton.addActionListener(e ->
        {
            ...
            {
                txtArea.append(String.format("%-30s%d\n",map.getKey(), map.getValue()));
            }
        });
        ...
        try
        {
            if (chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION)
            {
                ...
                while (inFile.hasNextLine())
                {
                    ...
                    txtArea.setText("File name: " + name + "\n\n\nThe Frq and the Keywords are:\n\n");
                    String word[] = readLine.split(" ");
                    for (int i = 0; i < word.length; i++)
                    {
                        String current = word[i];

                        if (keywordFrq(current, keyWord))
                        {
            ...
            {
                txtArea.setText("File not found, please choose a file.");
            }
        }
        catch (IOException e)
        {
